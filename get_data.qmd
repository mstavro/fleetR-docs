...
Note that, as of the time of writing, there's currently a [bug in httr2 where the token bucket in ```req_throttle``` gets reset every time the request function is called](https://github.com/r-lib/httr2/issues/801), so you may still run into 429 limits until a fix is issued. In the meantime, see the commentary in that GitHub issue for ways to get around the bug.
:::
