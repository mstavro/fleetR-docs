...
Here, ```tbl$destination_plausible``` is missing, . 

Other errors you might encounter include errors thrown by ```st_as_sf()``` about the absence of geometries and ```left_join()``` about a lack of columns to join, likely because no aircraft were returned from our API functions; an unlucky draw. Again, you can mitigate this by increasing the number of aircraft you pull.

::: {.callout}
Note that if you *do* increase the number of aircraft you want to pull, you'll likely need to add some cooldown time between your API requests, else you'll run into a rate limit of the OpenSky API. In our box analogy, the OpenSky team doesn't want you rolling up to their box, scooping up a bunch of stuff and then running off with it. It's almost Halloween as of the time of writing, and it wouldn't be very courteous of you to take the whole bowl of candy when there's a line of people taking only a few pieces at once. The same goes for our API access. 
